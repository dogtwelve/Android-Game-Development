...
public class CamTest extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//initialize Renderer
		newRenderer= new myRenderer();
		//initialize GLSurfaceView
		glSurfaceView = new GLSurfaceView(this);
		//set renderer
		glSurfaceView.setRenderer(newRenderer);
		//set listener
		glSurfaceView.setOnTouchListener(newRenderer);
...
float BackRectangle[]={0.0f,0.0f,1600.0f,0.0f,1600.0f,480.0f,0.0f,480.0f};
short BackRectangleIndex[]={0,1,2,2,3,0};
float BackRectangleTexturemap[]={0.0f,1.0f,1.0f,1.0f,1.0f,0.0f,0.0f,0.0f};
ByteBuffer byteBuffer;
FloatBuffer backvertices;
ShortBuffer backindex;
FloatBuffer backtexturemap;
...
boolean direction;
	public myRenderer(){
		...
		//first object buffers
				...
				backvertices = byteBuffer.asFloatBuffer();
				//copy float array to float buffer
				backvertices.put(BackRectangle);
				...
				backindex = byteBuffer.asShortBuffer();
				//copy short array to buffer
				backindex.put(BackRectangleIndex);
				...
				backtexturemap = byteBuffer.asFloatBuffer();
				//copy float array to float buffer
				backtexturemap.put(BackRectangleTexturemap);
				//set limits from 0 to number of floats
				backtexturemap.flip();
				//open external image on assets
				try {
						AssetManager assetManager = getAssets();
						InputStream inputStream = assetManager.open("back512x256.png");
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		//initialize array
		int backtextureIds[] = new int[1];
		//generate 1(n) texture in videomem return handle to textureIds[0]
		gl.glGenTextures(1, backtextureIds, 0);
		//get handle from array to an int
		backtextureId = backtextureIds[0];
		//bind using array
	...
	public void onDrawFrame(GL10 gl) {
		...
		gl.glVertexPointer(2,GL10.GL_FLOAT,8,backvertices);
		//pointer textmap(num_of_coord2 xy,dataFLOAT,stride,array)
		gl.glTexCoordPointer(2, GL10.GL_FLOAT,8, backtexturemap);
		//indexed draw
		gl.glDrawElements(GL10.GL_TRIANGLES, 6, GL10.GL_UNSIGNED_SHORT, backindex);
		...
		if(touchX<posx){velx=-150; direction=false;}
		if(touchX>posx){velx=150; direction=true;}
		//if no finger on screen vel=0
		if(!touch){velx=0;}
		//if object reaches touch point stop moving 
		if((touchX-centerx)*(touchX-centerx)<=radius1*radius1){velx=0;}
		//if reaches screen border stop
		if(posx<0){velx=0;}if(posx>1600){velx=0;}
		...
		if(direction){//offset, image is not centered
		centerx=posx-40;
		centery=posy-60;}
		else if(!direction){
		centerx=posx+40;
		...
		if(pos2x<0){vel2x=250;}if(pos2x>1600){vel2x=-250;}
		//check if posy is on the fixed ground 100
	...
	float camx,camy;
	public void updateCam(float cx, float cy, GL10 gl){ 
		camx=cx; camy=cy;
		//set projection with camx,camy offset
		gl.glMatrixMode(GL10.GL_PROJECTION);
		gl.glLoadIdentity();
		gl.glOrthof(0+cx, 800+cx, 0+cy, 480+cy, 1, -1);
		//set ModelView active for next operations
		gl.glMatrixMode(GL10.GL_MODELVIEW);
		gl.glLoadIdentity();
	}
