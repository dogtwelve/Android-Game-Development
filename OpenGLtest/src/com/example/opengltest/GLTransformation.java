...
public class GLTransformation extends Activity{
...
boolean dirx=true,diry=true;
	...
	public void onDrawFrame(GL10 gl) {
		...
		//load a clean matrix
		gl.glLoadIdentity();
		//first rotate second translate
		//translate to x,y
		gl.glTranslatef(x,y,0);
		//check if x,y outside screen
		if(x<0 | x>800){dirx=!dirx;}
		if(y<0 | y>480){diry=!diry;}
		//change direction if outofscreen
		if(dirx){x++;}if(!dirx){x--;}if(diry){y++;}if(!diry){y--;}
		//last transformation declared first applied
		//rotate arround z
