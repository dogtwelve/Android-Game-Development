...
public class MultipleShaders extends Activity {
...
public class SurfaceRenderer implements GLSurfaceView.Renderer,OnTouchListener{
    ...
    String vertexShaderSource = 
          ...
          + "   mat4 navigationRotationM=mat4(navigationMatrix);                       \n" 
...
boolean VBOcreated=false;
   ...
   if(VBOcreated){ 
	   ...
	   if(shaderTimer>8){if(shaderProgram==program){shaderProgram=program2;}else{shaderProgram=program;}shaderTimer=0;}//Log.d("time",""+deltaTime);
	 //set program and get all locations
	    GLES20.glUseProgram(shaderProgram);
	    //return attribute location to handle(program,attribute name)
	    attributePositionLocation=GLES20.glGetAttribLocation(shaderProgram, "attributePosition");
	    ...
	    //return attribute location to handle(program,attribute name)
	    attributeTextureCoordLocation=GLES20.glGetAttribLocation(shaderProgram, "attributeTextureCoord");
	    //if not found handle returned with -1
	    ...
	    //return attribute location to handle(program,attribute name)
	    attributeNormalLocation=GLES20.glGetAttribLocation(shaderProgram, "attributeNormal");
	    //if not found handle returned with -1
	    ...
	    samplerHandle=GLES20.glGetUniformLocation(shaderProgram, "samplerTexture");
	    ...
	    projectionMatrixHandle = GLES20.glGetUniformLocation(shaderProgram, "projectionMatrix");
	    ...
	    //get Uniform Handle(program,uniformName) 
	    physicsMatrixHandle = GLES20.glGetUniformLocation(shaderProgram, "physicsMatrix");
	    //if Uniform location not found handle is returned -1
	    if (physicsMatrixHandle == -1) {
	        throw new RuntimeException("Could not get physicsMatrix location");    }
	    //get Uniform Handle(program,uniformName) 
	    navigationMatrixHandle = GLES20.glGetUniformLocation(shaderProgram, "navigationMatrix");
	    //if Uniform location not found handle is returned -1
     ...
     GLES20.glDrawArrays(GLES20.GL_TRIANGLES, frameOffset*(OBJ.vertices.length/3),OBJ.vertices.length/3);

     //Animation Frame update
     frameTimeCounter+=deltaTime;
     if(frameTimeCounter>frameDuration){
    	 frameOffset++;
    	 if (frameOffset==20){frameOffset=0;}
    	 frameTimeCounter=0;}
   ...
   //physics update
    ...
    GLES20.glUniformMatrix4fv(projectionMatrixHandle, 1, false, projectionMatrix, 0);
    Matrix.setIdentityM(navigationMatrix, 0);
    //comment next lines to limit navigation on loading
    //  Matrix.rotateM(navigationMatrix, 0, -Pitch, 1, 0, 0);
    //  Matrix.rotateM(navigationMatrix, 0, -Yaw, 0, 1, 0);
    //  Matrix.translateM(navigationMatrix, 0, -navX, -navY, -navZ);
    //set the Uniform data source(Handle,elements to read,false,source,offset)
  ...
  //get direction for forward, backward movement
    ...
    float lookHsensitivity=0.03f;
    	...
    	public class loaderThread extends Thread{
    		...
    		Log.e("Thread", "runing");  
    		OBJ.load("walk_000001.obj");
    	    objVerticesFloatBuffer = ByteBuffer.allocateDirect(OBJ.vertices.length * 4 * 20).order(ByteOrder.nativeOrder()).asFloatBuffer();
    	    objVerticesFloatBuffer.put(OBJ.vertices);
    	    objTxFloatBuffer = ByteBuffer.allocateDirect(OBJ.tx.length * 4 * 20).order(ByteOrder.nativeOrder()).asFloatBuffer();
    	    objTxFloatBuffer.put(OBJ.tx);
    	    objNormalFloatBuffer = ByteBuffer.allocateDirect(OBJ.normal.length * 4 * 20).order(ByteOrder.nativeOrder()).asFloatBuffer();
    	    objNormalFloatBuffer.put(OBJ.normal);  
    	    String f;
    	    for(int frame=2; frame<21;frame++){
    	    	if(frame<10){f="0"+String.valueOf(frame);}else{f=String.valueOf(frame);}
    	    	OBJ.load("walk_0000"+f+".obj");
    	    	objVerticesFloatBuffer.put(OBJ.vertices);
    	    	objTxFloatBuffer.put(OBJ.tx);
    	    	objNormalFloatBuffer.put(OBJ.normal); }
    	    objVerticesFloatBuffer.position(0);
    	    objTxFloatBuffer.position(0);
    	    objNormalFloatBuffer.position(0); 
